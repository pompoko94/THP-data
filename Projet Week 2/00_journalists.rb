...
#1
puts "counting : #{ary.count}\n\n"

#2
puts ary.min { |a, b| a.length <=> b.length }

#6
puts ary.sort_by(&:length)
#3
puts ary.find { |l| l.size == 6 }

#5
puts ary.sort

#
puts ary.each_with_index{|e,i| puts "Element: #{e}, Index: #{i}"}

#7
puts ary.index { |x| ["@epenser"].include?(x) }

sorted = list.sort
puts sorted
puts list.length
#(puts list.sort)
puts list.count { |s| s.include?('_') }
puts list.count { |s| s.match?('[A-Z]') }
puts list.min { |a, b| a.length <=> b.length }
#puts list.min_by(&:length)
puts list.sort_by{ |word| word.length }
puts list.index { |x| ["@epenser"].include?(x) }
